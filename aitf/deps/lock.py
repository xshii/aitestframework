"""Lock file (deps.lock.yaml) management."""

from __future__ import annotations

import logging
from datetime import datetime, timezone
from pathlib import Path

import yaml

from aitf.deps.acquire import sha256_file
from aitf.deps.config import DepsConfig, detect_platform
from aitf.deps.repo import get_head_commit
from aitf.deps.types import LockEntry, LockFile

logger = logging.getLogger(__name__)

DEFAULT_LOCK_FILE = "deps.lock.yaml"


def generate_lock(
    cfg: DepsConfig,
    cache_dir: Path,
    repos_dir: Path,
) -> LockFile:
    """Inspect installed dependencies and generate a lock file.

    Args:
        cfg: Parsed deps.yaml configuration.
        cache_dir: ``build/cache/`` directory where deps are unpacked.
        repos_dir: ``build/repos/`` directory where repos are cloned.

    Returns:
        A :class:`LockFile` reflecting current installation state.
    """
    now = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S")
    lock = LockFile(
        generated_at=now,
        platform=detect_platform(),
    )

    # Toolchains
    for name, tc in cfg.toolchains.items():
        install_dir = cache_dir / f"{name}-{tc.version}"
        if install_dir.is_dir():
            plat = detect_platform()
            sha = tc.sha256.get(plat, "")
            lock.toolchains[name] = LockEntry(
                name=name,
                version=tc.version,
                sha256=sha,
                installed_at=now,
            )

    # Libraries
    for name, lib in cfg.libraries.items():
        install_dir = cache_dir / f"{name}-{lib.version}"
        if install_dir.is_dir():
            lock.libraries[name] = LockEntry(
                name=name,
                version=lib.version,
                sha256=lib.sha256,
                installed_at=now,
            )

    # Repos
    for name, repo in cfg.repos.items():
        repo_dir = repos_dir / name
        if repo_dir.is_dir() and (repo_dir / ".git").exists():
            commit = get_head_commit(repo_dir)
            lock.repos[name] = LockEntry(
                name=name,
                ref=repo.ref,
                commit=commit,
                installed_at=now,
            )

    return lock


def save_lock(lock: LockFile, path: str | Path = DEFAULT_LOCK_FILE) -> None:
    """Write the lock file to disk."""
    p = Path(path)
    data: dict = {
        "generated_at": lock.generated_at,
        "platform": lock.platform,
    }
    if lock.toolchains:
        data["toolchains"] = {
            name: _entry_dict(e) for name, e in lock.toolchains.items()
        }
    if lock.libraries:
        data["libraries"] = {
            name: _entry_dict(e) for name, e in lock.libraries.items()
        }
    if lock.repos:
        data["repos"] = {
            name: _repo_entry_dict(e) for name, e in lock.repos.items()
        }

    with open(p, "w", encoding="utf-8") as fh:
        yaml.dump(data, fh, default_flow_style=False, allow_unicode=True, sort_keys=False)
    logger.info("Lock file written: %s", p)


def load_lock(path: str | Path = DEFAULT_LOCK_FILE) -> LockFile | None:
    """Load an existing lock file, or return ``None`` if missing."""
    p = Path(path)
    if not p.exists():
        return None
    with open(p, encoding="utf-8") as fh:
        data = yaml.safe_load(fh) or {}

    lock = LockFile(
        generated_at=data.get("generated_at", ""),
        platform=data.get("platform", ""),
    )
    for name, raw in data.get("toolchains", {}).items():
        lock.toolchains[name] = LockEntry(
            name=name,
            version=raw.get("version", ""),
            sha256=raw.get("sha256", ""),
            installed_at=raw.get("installed_at", ""),
        )
    for name, raw in data.get("libraries", {}).items():
        lock.libraries[name] = LockEntry(
            name=name,
            version=raw.get("version", ""),
            sha256=raw.get("sha256", ""),
            installed_at=raw.get("installed_at", ""),
        )
    for name, raw in data.get("repos", {}).items():
        lock.repos[name] = LockEntry(
            name=name,
            ref=raw.get("ref", ""),
            commit=raw.get("commit", ""),
            installed_at=raw.get("installed_at", ""),
        )
    return lock


def _entry_dict(e: LockEntry) -> dict:
    d: dict = {"version": e.version}
    if e.sha256:
        d["sha256"] = e.sha256
    d["installed_at"] = e.installed_at
    return d


def _repo_entry_dict(e: LockEntry) -> dict:
    d: dict = {"ref": e.ref, "commit": e.commit}
    d["installed_at"] = e.installed_at
    return d
